		...
		if (cap.isOpened()) {
			...
			while (keepProcessing) {
				...
				if (!(matFrame.empty())) {
					...
					HOG.detectMultiScale(matFrame, foundLocations, foundWeights, 0, new Size(8, 8), new Size(32, 32),
							1.05, 8, false);
					...
					for (int i = 0; i < rectangles.size(); i++) {
						...
						Imgproc.rectangle(matFrame, new Point(rectangles.get(i).x, rectangles.get(i).y),
