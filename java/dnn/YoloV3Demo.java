...
public class YoloV3Demo {
    ...
    private static List<String> getOutputsNames(Net net)
    {
        ...
        return net.getUnconnectedOutLayers().toList().stream().map(layersNames::get).collect(Collectors.toList());
    }
    ...
    private static void postprocess(String filename, Mat frame, List<String> labels, List<Mat> outs, List<Integer> classIds, List<Float> confidences, List locations, int nResults)
    {
    ...
    private static void annotateFrame(Mat frame, List<String> labels, List<Integer> classIds, List<Float> confidences, int nResults, List<Rect> tmpLocations, List<Integer> tmpClasses, List<Float> tmpConfidences) {
        ...
        for (int i = 0; i < indexMat.total() && i < nResults; ++i)
        {
            ...
            Imgproc.putText(frame, label, new Point(box.x, box.y), Imgproc.FONT_HERSHEY_PLAIN, 1.0, new Scalar(0,0,0),2);
